...
main = do
    ...
    print $ alignment_simple (init contents1) (init contents2)

alignment_simple :: String -> String -> Int
alignment_simple seq1 seq2 = simple seq1 seq2 ([0]++(take (length seq2) [gap,gap*2..])) 0

simple :: String -> String -> [Int] -> Int -> Int
simple seq1 seq2 top_line cont
    | cont == (length seq1) = last top_line
    | otherwise = simple seq1 seq2 (new_row' ([(top_line!!0) + gap]) top_line seq1 seq2 cont) (cont+1)
...
calculate_value left top_left top s1_c s2_c
    | s1_c == 'N' || s2_c == 'N' = minimum $ [left + gap, top + gap, top_left]
    | otherwise = minimum $ [left + gap, top + gap, top_left + (mismatch!!(order s1_c))!!(order s2_c)]
